...
describe('chatHistoryStore', () => {
  ...
  it('loadHistory: JSON inválido → [] (degrada, sem lançar)', async () => {
    mockedAsyncStorage.getItem.mockResolvedValue('{ não é json')
    expect(await loadHistory()).toEqual([])
  })
  ...
  it('saveHistory: corta para CHATBOT_MAX_HISTORY (mantém as mais recentes)', async () => {
    ...
    const [key, payload] = mockedAsyncStorage.setItem.mock.calls[0]
  ...
  it('saveHistory: falha de escrita não lança', async () => {
    mockedAsyncStorage.setItem.mockRejectedValue(new Error('disk full'))
