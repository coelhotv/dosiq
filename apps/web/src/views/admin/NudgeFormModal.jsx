...
function validateBasicContent(formData, newErrors) {
  if (!formData.title) {
  ...
  if (!formData.body) {
    newErrors.body = 'Corpo obrigatório'
  } else if (formData.body.length > 200) {
    newErrors.body = 'Máximo 200 caracteres'
  }
}

function validateDates(formData, newErrors) {
...
function validateActionPayload(formData, newErrors) {
  if (formData.action_type === 'navigate') {
    ...
    if (!formData.action_payload?.url) {
      newErrors.action_payload = 'URL obrigatória'
    } else if (!isValidUrl(formData.action_payload.url)) {
      newErrors.action_payload = 'URL inválida'
    }
  }
}

function validateNudge(formData) {
  const newErrors = {}
  validateBasicContent(formData, newErrors)
  validateDates(formData, newErrors)
  validateActionPayload(formData, newErrors)
  return newErrors
}

function NudgeContentFields({ formData, handleChange, errors }) {
  return (
...
function NudgeTargetingFields({ formData, handleChange }) {
  return (
    <fieldset className="nudges-admin__form-section">
...
function NudgeNavigateActionFields({ formData, setFormData, handleActionPayloadChange, errors }) {
  return (
    <fieldset className="nudges-admin__form-section">
...
function NudgeOpenUrlActionFields({ formData, handleActionPayloadChange, errors }) {
  return (
    <fieldset className="nudges-admin__form-section">
      <legend>Ação Específica</legend>
      <div className="form-group">
...
function NudgeDismissActionFields({ formData, handleActionPayloadChange }) {
  return (
    <fieldset className="nudges-admin__form-section">
      <legend>Ação Específica</legend>
      <div className="form-group">
...
function NudgeActionSpecificFields({ formData, setFormData, handleActionPayloadChange, errors }) {
  if (formData.action_type === 'navigate') {
    return (
      <NudgeNavigateActionFields
        formData={formData}
        setFormData={setFormData}
        handleActionPayloadChange={handleActionPayloadChange}
        errors={errors}
      />
    )
  }
  ...
  if (formData.action_type === 'dismiss_only') {
    return (
      <NudgeDismissActionFields
        formData={formData}
        handleActionPayloadChange={handleActionPayloadChange}
      />
    )
...
function NudgePeriodFields({ formData, handleChange, errors }) {
  return (
    <fieldset className="nudges-admin__form-section">
...
function NudgeVersionFields({ formData, handleChange }) {
  return (
    <fieldset className="nudges-admin__form-section">
...
export default function NudgeFormModal({ nudge, isOpen, onClose, onSubmit, isLoading }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const newErrors = validateNudge(formData)
    ...
    <div className="nudges-admin__modal-overlay" onClick={onClose}>
      ...
      <div className="nudges-admin__modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="nudges-admin__form">
          ...
          <NudgeTargetingFields
            formData={formData}
            handleChange={handleChange}
          />

          <NudgeActionSpecificFields
            formData={formData}
            setFormData={setFormData}
            handleActionPayloadChange={handleActionPayloadChange}
            errors={errors}
