...
describe('protocolService', () => {
  ...
  // 029 F3.1 (T017i): removida a suíte de `advanceTitrationStage` — a função foi deletada do
  // repositório. Era write-path da web p/ `titration_status`/`stage_started_at`, sem NENHUM
  // chamador em produção: a função que o AP-301 indicia por nome ("dívida ativa, não código
  // inofensivo"). A escada nasce e avança no app + cron, sobre `titration_steps`.
})
