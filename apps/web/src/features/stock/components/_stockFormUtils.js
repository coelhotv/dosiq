...
const validateLiquidFields = (formData, errors) => {
  ...
  if (Number.isNaN(volume) || volume <= 0) {
    errors.volume_per_bottle = 'Informe o volume por frasco em ml (maior que zero)'
  }
  if (formData.total_price && isNaN(coerceDecimal(formData.total_price))) {
    errors.total_price = 'Deve ser um número'
  }
}
...
const validateSolidFields = (formData, errors) => {
  ...
  if (formData.unit_price && isNaN(coerceDecimal(formData.unit_price))) {
    errors.unit_price = 'Deve ser um número'
  }
}
...
export const validateStockForm = (formData, isLiquid = false) => {
  ...
  if (isLiquid) {
    validateLiquidFields(formData, newErrors)
  } else {
    validateSolidFields(formData, newErrors)
  }

  validateDateFields(formData, newErrors)

  return newErrors
}

const _buildLiquidPayload = (base, formData) => {
...
const _buildSolidPayload = (base, formData) => {
  return {
    ...base,
...
export const buildStockPayload = (formData, effectiveLaboratory, isLiquid = false, isInjectable = false) => {
  ...
  return isLiquid
    ? _buildLiquidPayload(base, formData)
    : _buildSolidPayload(base, formData)
}
