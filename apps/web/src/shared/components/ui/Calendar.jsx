...
export default function Calendar({
  ...
  // setDate(1) antes de setMonth: evita o overflow clássico do Date (ex: 31/mai − 1 mês
  // = "31/abr" → transborda p/ 01/mai, travando a navegação). Gemini #621.
  ...
  const canGoNext = !isAfterMax((() => { const n = cloneDate(viewDate); n.setDate(1); n.setMonth(n.getMonth() + 1); return n })())
  const canGoPrev = !isBeforeMin((() => { const n = cloneDate(viewDate); n.setDate(1); n.setMonth(n.getMonth() - 1); return n })())

  const handlePreviousMonth = () => setViewDate((d) => { const n = cloneDate(d); n.setDate(1); n.setMonth(n.getMonth() - 1); return isBeforeMin(n) ? d : n })
  const handleNextMonth = () => setViewDate((d) => { const n = cloneDate(d); n.setDate(1); n.setMonth(n.getMonth() + 1); return isAfterMax(n) ? d : n })
