...
### Web/PWA
- **SparklineAdesao: clamp + auto-escala + a11y + drift** (Patch, PR-F3.2c, web 3.6.0→3.6.1): o gráfico "Adesão 30 Dias" (HealthHistory) (1) **clampa adesão 0-100** no componente — para de exibir `>100%`/"118%↓" mesmo se a fonte mandar errado (defesa AP-191; raiz na view fica p/ F4/G6); (2) **auto-escala vertical** (`computeDomain`/`projectY`, domínio min-max com margem, path e dots no mesmo domínio) — tira a curva do "bloco chapado" quando os valores variam pouco; (3) **acessibilidade** restaurada: container `role="img"` + dots `role="button"`/`aria-label`/teclado (Enter/Espaço); (4) **corrige o drift do tooltip no hover** — `<circle>` SVG escalava a partir da origem do viewport (`transform-box: fill-box; transform-origin: center`). 10 testes de contrato (a11y/tooltip/drill-down) verdes.
...
- **Deprecação de Gemini Reviews** (`no-user-impact`, PR #607): Removidos barrel exports de `geminiReviewSchema` de `packages/core/src/schemas/index.js` para isolar a limpeza estrutural da persistência de reviews de IA no banco.
