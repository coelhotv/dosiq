...
- **[AP-192]** SPA reseta a view no refresh de token (refocus da aba) — effect de roteamento URL→view depende de `session`; ao refocar, Supabase renova o token → novo `session` → effect re-dispara → como a URL não acompanha navegação interna (SPA state-based), força `setCurrentView('dashboard')` → usuário é jogado do HealthHistory pro "hoje". Gatear effect à IDENTIDADE do usuário (login/logout), não a toda atualização de `session`. -> [`anti-patterns/react_and_ui/AP-192.md`](./anti-patterns/react_and_ui/AP-192.md)
