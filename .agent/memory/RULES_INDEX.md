...
- **[R-270]** DB Change Preflight (migração/RPC/função SQL): tabela obrigatória de Failure Modes & Degenerate Inputs (NULL de cada arg, divisor 0, dose→0, JOIN/FK ausente) + semântica SQL NULL-safe + integridade estrutural (CHECK, diff `pg_get_functiondef`, SECURITY DEFINER, idempotência) + teste por failure mode (BEGIN..ROLLBACK). Substitui o gate de revisor externo por disciplina de processo -> [`rules/data_and_schema/R-270.md`](./rules/data_and_schema/R-270.md)
