...
- [ ] T003 [US1] Mesma migração — A.2 colunas (`medicines.drops_per_ml`, `protocols.intake_unit`) + `CHECK (stock.quantity >= 0)`.
- [ ] T004 [US2] Mesma migração — A.3 migração de dados (`ml`/`gotas` → `mg/ml` + `protocols.intake_unit`), idempotente.
...
- [ ] T009 [US4] `medicineSchema.js`: enum `+ mg/ml,ui/ml`, `drops_per_ml`/`dosage_per_pill` nullable, superRefine líquido.
...
- [ ] T016 [US6] `MedicineForm.jsx` (+ `MedicineFormDosageInfo.jsx`) + `MedicineFormScreen.jsx` (mobile): dropdown novo, badge `💧`, campo `Gotas por ml`.
...
- [ ] T020 [US8] `StockAlertInline.jsx`: `nextDoseMl` (gotas→ml via `drops_per_ml`) + banner quando `stock.quantity < doseMl`.
...
## Traceability
FR-001..006 → Fase A (T002–T007) · FR-007..012 → Fase B (T009–T014) · FR-013..017 → Fase C (T016–T023).
