...
| 073 | fix-clinical-data | **specified** | Guarda-chuva dos furos do documento clínico: 16 achados (`F-1..F-16`) com `file:line`, em 3 superfícies só-web. PDF chama injetável/gotas de "comprimido" e semanal de "1x/dia"; status de prescrição tem 2 implementações (core 14d × web 30d); **modo consulta calcula dose diária como produto cruzado e dobra a posologia com 2+ tratamentos**; **cartão de emergência lista tratamento encerrado e colapsa 2 tratamentos do mesmo medicamento em 1**. Medido em prod: 9 protocolos vencidos ainda "ativos", 2 medicamentos com 2 tratamentos, 15 vigentes não-diários. 5 slices (A PDF · B prescrição · C higiene · D emergência · E consulta); prioridade clínica sugere E e D primeiro. §8 confirma no banco que não existe tabela `prescriptions` — evidência p/ o item 5.7 do backlog H2, que segue com gate próprio. §9: o cartão passou no sweep de FORMATAÇÃO e falhou no de SELEÇÃO/AGREGAÇÃO — sweep de rótulo não é sweep de dado. Bloqueia a `007` e a `005` |
