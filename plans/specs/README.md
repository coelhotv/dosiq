...
| 064 | stock-forecast-vigency | **in-progress (PR 2/2 em review)** | PR #807 (`72d5e322`) entregou o predicado canônico. PR 2 aberto: `asOf` opcional em `calculateDailyIntake`/`stockDoseMetrics` (com o `rep` saindo da lista filtrada), 8 superfícies convergidas, PDF clínico pela data de referência do relatório (F-14 opção (a)) e gate `scripts/vigency-single-source.sh` no `lint`. Tier 2, sem migração. Eixo `paused_at` fora ([[AP-290]]). Desbloqueia a `007` |
