...
| 050 | notification-integrity | **planned** | Follow-up da 043. Fan-out da outbox (stock_alert) + claim atômico do lembrete. US2 rebaixada P1→P3 (PO-5 fechado: zero duplicatas medidas em prod). Plano em **5 PRs** pós-RC3 (0 US4 · 1a infra · 1b fan-out · 2 expiry+cutover · 3 claim); droppabilidade 3→2→1b→1a, PR 0 nunca. **US4 add 2026-08-02** (bug ativo em prod: alerta de estoque de tratamento encerrado/pausado — corrigido no PR 1, só no caminho de push; raiz na spec 064) |
...
| 064 | stock-forecast-vigency | **draft** | Tier 2 provável. Raiz do bug recortado na US4 da 050: `calculateDailyIntake` (`adherenceLogic.ts:349`) filtra só `p.active` — ignora `end_date`/`paused_at` ⇒ tratamento encerrado segue contando como consumo vivo em 7 superfícies (tela de estoque, dashboard, `stockDoseMetrics`, custo, `/estoque` do bot, chatbot, PDF de consulta). Prod 2026-08-02: 4 protocolos `active` com `end_date` vencida, 3 usuários. A 050 corrigiu só o caminho de PUSH. ⚠️ O que dimensiona a spec é o **PDF de consulta**: ele descreve período possivelmente PASSADO, onde "vigente hoje" reescreveria o histórico (R-299) — precisa de vigência por data de referência, não de hoje. NC1 (assinatura: parâmetro `asOf` vs função nova) e NC2 (PDF muda ou fica intocado) decidem o Tier. Sem migração. NÃO planejado |
| 065 | analytics-instrumentation-mobile | **specified** | Tier 2 (por privacidade/LGPD, não por schema — não toca banco nem UI). Fase 1 do projeto de analytics: fecha o **mobile** por completo sob [[CON-034]]/[[ADR-090]] — 15 órfãos cabeados, `surface`+`treatment_id` (do FATO, R-299), super property `mode`, famílias novas (tratamento/biomarcador/IA/perfil). Baseline PostHog 2026-08-09: `surface`/`treatment_id` ausentes em 100%, PII limpo ⇒ instrumentação é aditiva. Promovida do draft em 2026-08-12 com 3 deltas materiais: **US8/FR-11** (durabilidade em handler de background — o `capture` faz batch em memória sem flush; o caso `surface:push` é o mais valioso de medir e o mais fácil de perder), SC-7 (não-regressão dos 7 eventos que já chegam) e isenção nominal de `dev_smoke_event`. **3 NC fechados pelo PO em 2026-08-13** (Decisões 4/5/6): PostHog único mitigado por super property `app_env`+`is_internal` (FR-12, sem infra nova); durabilidade = **nenhum mecanismo novo; medir primeiro** (D5 revista na mesma data) — o SDK já persiste a fila em arquivo e preserva o `timestamp` do fato, então o risco é *atraso*, não perda; `flush()` compraria só pontualidade contra problema não-medido, ao custo de código no caminho headless (AP-316/AP-318). FR-11 vira obrigação de MEDIR (PO-8: latência por plataforma), mitigação é follow-up gatilhado pelo número; `stock_*` passa a `medicine_id` (estoque é do medicamento, não do tratamento). Zero markers abertos. **RC3 rodada 2026-08-13** (6 findings, 1 critical): `surface` e mudanca de ASSINATURA de `registerDose` (funil unico, 9 importadores, serve UI e push) — default `'mobile'` proibido; complexity check estourou (~25 arq vs 8) -> fatiado por valor em **PR A/B/C/D** (A sozinho responde a tese; droppabilidade D->C->B->A nunca); +FR-13 (evento nasce no service) e FR-14 (`delete_orphan` deixa de ser cego). Guards PO-1/PO-2 elevados a full. **planned 2026-08-13** (plan.md + tasks.md, 30 tasks / 4 PRs). Achados do planning: `registerDose` JA tem objeto de opcoes (surface entra additivo); call-sites reais sao 5 arquivos/8 chamadas (nao 9); `medicineService`/`protocolService`/`stockService`/`measuresRepo` sao repositorios do core reexportados -> emit vai em casca fina, que ADR-071 ja prescreve, zero mudanca em packages/core; DOIS caminhos de logout. 🔴 BLOQUEADO em T001: `posthog-cli` ausente e `.env` so tem a chave publica de escrita -> 5 das 8 POs sem canal de prova |
| 066 | camera-medicine-scan | **specified** | Tier 2 — épico multifases nascido do spike `plans/spikes/camera-medicine-capture.md` (D1–D11). **Mobile apenas.** Tese: o gargalo do 1º cadastro não é captura, é DADO — a base ANVISA em prod (6.976 linhas, 5 campos) **não tem dosagem, apresentação, quantidade nem EAN**, então nem OCR perfeito preencheria o form. A lista **CMED** (mesma ANVISA, dataset outro) traz 26.001 apresentações com EAN/APRESENTAÇÃO/LABORATÓRIO/REGISTRO/TARJA em **~603 KB gzip** — cabe no on-demand do [[CON-027]] (extensão aditiva: `fileKey` ganha `'cmedIndex'`), **sem infra nova**. Arquitetura em cascata **L0 EAN-13 → L1 OCR → L2 manual**; **sem LLM** (parser determinístico cobre **92,0%**, e Foundation Models/Gemini Nano recortariam a base instalada BR). **Fase 0** (índice+parser+migração, zero código nativo) é entregável autônomo; **Fase 1** = scan via `expo-camera` (sem vision-camera/worklets). Fases 2 (OCR, travada até a 055 fechar) e 3 (preço/tarja/classe→018) fora de escopo. 🔴 Guardas clínicas medidas: **5,5%** dos pares compostos têm ordem de dose **indecidível na fonte** (`CONCOR ANLO (5+10)` e `(10+5)`, mesmo produto e lab) ⇒ não auto-preencher dose; e **laboratório não é identidade de genérico** (clopidogrel = 17 labs, 0 de 8.984 prefixos de REGISTRO cruzam fabricante) ⇒ identidade varia por categoria regulatória, senão vira 17 cadastros do mesmo remédio. Migração aditiva (`medicines.anvisa_registration`, `purchases.ean`, `purchases.funding_source`+CHECK) + backfill que discrimina por **`pharmacy`, não por preço** (varejo tem 0 compras zeradas; usuários já usam `pharmacy` como campo de custeio). **3 NC abertos** (NC-1 onde vive o pipeline · NC-2 como a Fase 0 entrega valor no autocomplete, toca [[CON-012]] · NC-3 ponto de entrada do scan na UI). Aguarda decisão do PO + ceremonies |
