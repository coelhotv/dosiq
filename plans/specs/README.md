...
## Tabela de status (atualizada 2026-06-23)
...
| 015 | ai-chatbot-mobile | **planned** | Tier 2 (RC3 done). plan+tasks+analysis(stub)+checklist prontos. ADR-074 (accepted) + CON-028 (proposed). 4 ondas: 1a (core fetcher+builder, web+server adotam, paridade) · 1b (plan-grouping) · 2 (chat mobile e2e + FR-012 loading branded) · 3 (polish). Fetcher canônico mata drift de input; web E mobile já fazem join treatment_plan nos protocols (leverage). Aguarda coding |
...
| 031 | injection-site-rotation | **delivered** | PR #675 mergeado 2026-06-22 (web `4.10.0` · mobile `0.20.0`). 10/10 POs fechados. A: captura+rotação global+histórico+migração `20260621`. B: edição pós-log (web LogForm + mobile DoseActionSheet p/ qualquer status, missed→retroativo), bulk per-item (canetas não compartilham sítio), hint absorção mobile, paridade US2/US3 mobile (última aplicação + alerta rotação), emoji→lucide. ADR-072 accepted; CON-026 +param `p_injection_site` |
...
| 035 | unified-dose-log-stock-core | **delivered** | PR #673 mergeado 2026-06-19. **Option A (atomicidade real, ADR-071)**: 3 RPCs transacionais (`register`/`update`/`delete_dose_log_atomic`, migração `20260619`) compõem `consume_stock_fifo`/`restore_stock_for_log` numa única transação → mata classe AP-231 na origem (sem rollback compensatório JS). `createDoseLogService` (CON-026) unifica core; web `logService` + mobile `doseService` viram cascas finas. Ancoragem estrita (double-click aborta txn) vs best-effort; tolerância unificada (120); `update` com flags de presença p/ limpar nullable→NULL. Review Gemini: AP-236 (instanceId em lote web+mobile), destruct. defensiva. validate:agent 1367 verdes |
...
| 037 | anvisa-web-ondemand | planned | Tier 2 (upgraded via RC3) — hoist da lógica on-demand p/ `@dosiq/core` (storage adapter web=Cache Storage / mobile=AsyncStorage) matando triplicação; JSONs → `data/anvisa/` (raiz, fora do build); FR-008 caches-guard, FR-009 Workbox. 2 slices (1 core+web, 2 mobile). RC3 done. Web tira ~1.36 MB do build PWA. Cobre `medicineDatabase`+`laboratoryDatabase`; Cache Storage + manifest version + TTL 7d + timeout (R-168) + degradação offline. API services intacta (CON-012 não-breaking). + guia de operações + bump mobile (R-221). Aguarda planning |
| 038 | refactor-web-structure | **delivered** | Decisão **A + carve-out** (views/ = camada de composição legítima, R-279). 4 slices mergeados 2026-06-23: **A** #677 (`git mv views/redesign/*`→`views/` + Landing→`views/landing/` + alias `@settings` + manualChunks + AppViewRouter; AP-238), **B1** #678 (dead code: 7 bases pré-redesign + renames collision-free; −2843 linhas), **B2** #679 (dissolve 4 pastas `features/*/components/redesign/` + 6 renames base-morta; AP-239), **C** #680 (carve-out `deriveProtocolStatus`→`@dosiq/core::derivePrescriptionStatus` +7 testes + R-279 + limpa `views/*Redesign` + doc CLAUDE.md; Gemini pegou bug HIGH date-window→AP-240). `apps/web/src` com 0 arquivos/pastas `*Redesign`. **Resta só** classes CSS BEM `*-redesign__*` (cosmético, opcional). Refator puro, sem mudança de comportamento |
