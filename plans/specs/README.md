...
| 064 | stock-forecast-vigency | **in-progress (PR 1/2)** | PR #807 (`72d5e322`) — predicado canônico `isProtocolVigentOn` no core, `ProtocolSchema` deixa de estripar `start_date`/`end_date` e sombra morta de `calculateDailyIntake` removida; **zero mudança de comportamento** por construção. Falta o **PR 2** (`asOf` em `calculateDailyIntake`/`stockDoseMetrics`, 6 superfícies + PDF clínico e o gate de fonte única). Tier 2, sem migração. Eixo `paused_at` fora ([[AP-290]]). Desbloqueia a `007` |
