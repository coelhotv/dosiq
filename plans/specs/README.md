...
| 050 | notification-integrity | **in-progress** | **PR 0 mergeado (#799, squash `208327b`, 2026-08-19)** — vigência do tratamento no alerta de estoque. **PR 1a em review** — `notification_outbox.subject_id` + UNIQUE de 4 colunas `NULLS NOT DISTINCT` aplicada em prod (infra pura, zero consumidor, zero backfill; ADR-078 emendado, CON-035). Follow-up da 043: fan-out da outbox (`stock_alert`, precisa de `subject_id`) + claim atômico do lembrete. Plano em 5 PRs (0 US4 · 1a infra · 1b fan-out · 2 expiry+cutover · 3 claim), droppabilidade 3→2→1b→1a, PR 0 nunca. US4 = bug ativo em prod (alerta de tratamento encerrado, **7 protocolos / 5 usuários** em 19/08 — era 4/3 no planning); raiz na 064 |
