...
| 031 | injection-site-rotation | **in-progress** | 031-A+031-B implementados (web 4.10.0 · mobile 0.20.0). 10/10 POs fechados (smoke aprovado). 031-A: captura+rotação global+histórico+migração. 031-B: edição pós-log (web LogForm + mobile DoseActionSheet p/ qualquer status, missed→retroativo), bulk per-item (canetas não compartilham sítio), hint absorção mobile, paridade US2/US3 mobile (última aplicação + alerta rotação), emoji→lucide. ADR-072 accepted; CON-026 +param. Aguarda PR/merge |
