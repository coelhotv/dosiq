...
| 037 | anvisa-web-ondemand | specified | Tier 1 — ANVISA web on-demand do Supabase Storage público (`dosiq-assets/anvisa/v1/`, paridade com mobile `useMedicineDatabase`), Cache Storage API + manifest version + TTL 7d + fetch com timeout (R-168) + degradação graciosa offline. Cobre `medicineDatabase`+`laboratoryDatabase`; remove ~1.36 MB de JSON do build PWA (import + `manualChunks` vite). API dos services intacta (CON-012 não-breaking). Inclui atualização do guia de operações (web como consumidor; JSONs do repo = fonte de upload). Web-only; aguarda planning/coding |
| 038 | refactor-web-structure | specified | Tier 1 (refactor) — aposentar naming `redesign` (entregue): `git mv views/redesign/*`→`views/` + dissolver `features/*/components/redesign/`; atualizar 26 importers + alias `@settings` + 3 `manualChunks` (R-117) + lazy `AppViewRouter`. Agrupar Landing em `views/landing/`; documentar `features/measures` no CLAUDE.md. Refator puro, sem mudança de comportamento. 2 [NEEDS CLARIFICATION] (camada views: composição vs wrapper; escopo FR-002). Guard: build + bundle gzip ±5% + test:critical. Web-only; aguarda resolução das clarifications + planning/coding |
