...
| 035 | unified-dose-log-stock-core | **specified** | Tier 2 — convergência web↔mobile: extrair orquestração de registro de dose (`logService`/`doseService`) para `createDoseLogService` no core. **Pré-req:** unificar estoque web (app-side+delete compensatório) → RPC `consume_stock_fifo` atômico (mata classe AP-231). Nascida da auditoria da 033. |
