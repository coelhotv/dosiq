...
| 067 | critical-alarm-window-guard | **in progress** — A1 ✅ merged (#795, `ca673de6`) · A2 ✅ merged (#796, `72653942`) · **B1 ✅ merged** (#797, `65931d11`, 2026-08-19 — RPCs + 3 writers migrados, IDOR do bot fechado) · ⏸ **B2 SEGURADO** (o `REVOKE UPDATE(status)`: migração escrita, aplicação aguarda adoção da v0.31.1 nas lojas — gatilho em `plan.md` §B2, gate = PO-SEC-2) · **C próxima** (absorveu a higiene do trail: 79% de `dose_critical_events` é ruído de Live Activity em Android — FR-040/041, PO-17/18; **não depende do B2**) | Tier 2 — dose crítica de 13:30 disparou alarme às 09:52 e virou `skipped_user` (prod 2026-08-14). A1 visibilidade → A2 guarda+telemetria → B1 skip por RPC (ADR-092) → **B2 REVOKE (pendência ativa)** → C *fired* vs *observed*. A1 nunca dropável |
