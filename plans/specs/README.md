...
| 007 | medical-pdf-report | **planned** | 📌 A geração **web existe** (`features/reports/consultationPdfService.ts` + builder + testes), do **legado pré-specs**; o escopo desta spec é o **porte mobile** (`expo-print`) — promovido para a Onda 2 (2.8b) em 2026-08-21. 🔴 Bloqueado pela `064`: o builder importa `calculateDailyIntake` ⇒ o PDF já imprime vigência errada. Absorve Fase E da 012 |
...
| 064 | stock-forecast-vigency | **planned** | Tier 2, sem migração, **2 PRs** (estrutura → comportamento). `calculateDailyIntake` (`adherenceLogic.ts:341`) filtra só `p.active` e ignora `end_date` ⇒ tratamento encerrado conta como consumo vivo — inclusive no **PDF clínico entregue ao médico**. Bundle completo + RC3 em 2026-08-21; eixo `paused_at` REMOVIDO (é carimbo derivado de `active=false`, [[AP-290]]) e a lista da tela de estoque saiu do escopo de sintoma (`getActive` já filtra). Desbloqueia a `007` |
