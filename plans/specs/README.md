...
| 036 | fix-alarm-stale-snooze | **in-progress** | PR #672 (mobile `0.19.1`) aberto, aguarda review+merge. Tier 1 hotfix — (1) alarme de soneca/nag dispara obsoleto após dose resolvida por outra superfície + full-screen sem saída (bug prod 0.19.0, desde soneca v1; 025 fechou só metade): cancel-on-resolve em `registerDose`/`Many` + auto-dismiss `AlarmFullScreen`; (2) **risco clínico**: full-screen passa a mostrar concentração + quantidade a tomar (reuso `formatMedicineConcentration`/`formatDoseItem` do core; corrige "un." hardcoded p/ líquido/injetável da 012). Mobile-only; spec+tasks prontos, aguarda coding |
| 035 | unified-dose-log-stock-core | draft | Tier 2 — convergência web↔mobile: extrair orquestração de registro de dose (`logService`/`doseService`) para `createDoseLogService` no core. **Pré-req:** unificar estoque web (app-side+delete compensatório) → RPC `consume_stock_fifo` atômico (mata classe AP-231). Nascida da auditoria da 033. Não iniciar sem priorização do PO |
