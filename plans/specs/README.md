...
| 015 | ai-chatbot-mobile | **specified** (RC3 done) | **Reescrita 2026-06-23 → Tier 2** (escopo ampliado, decisão PO). 3 eixos / 3 ondas (modelo MASTER_PLAN G1→G2→G3): (1) centralizar builder de contexto no `@dosiq/core` matando o fork web (`contextBuilder.js`) ↔ Telegram (`buildServerContext`); (2) melhorar contexto agrupando por plano terapêutico (`treatment_plans.name`); (3) port do chat p/ mobile consumindo o core. ADR-074 + CON-028 (proposed). 2 [NEEDS CLARIFICATION] (local no core; builder puro vs adapter). Aguarda eng-review/planning |
