...
| `plans/backlog-unified_app_2026/PHASE_7_COMMUNICATION_CUIDADOR.md` | `master_plan` | **Fase 7A consolidada no épico Tier 2 `plans/specs/009-caregiver-mode/`** (fases internas):<br>• `phase-1-foundation-rls/` (era 010)<br>• `phase-2-setup-flow/` (era 009)<br>• `phase-3-caregiver-dashboard/` (era 011)<br>• `phase-4-alert-engine/` (**NOVA** — engine de alertas)<br>• `phase-5-observer/` (era 012)<br><br>**GATE 0** = `plans/specs/002-caregiver-demand-teaser/` (standalone).<br>**Fase 7B (WhatsApp)** = épico próprio: `013-whatsapp-bot-adapter/` + `014-whatsapp-templates-webhook/`. | `migrated` | Reorganizado 02/06/2026: 009-012 colapsadas no épico `009-caregiver-mode` com cadeia de gates de produto, owner=paciente (D1), auth observer por token (D2) e nova phase-4 alert-engine. Vacam os números 010/011/012. 7B segue separado (Meta gated). |
| `plans/backlog-unified_app_2026/PHASE_8_SMART_WOW_FACTOR.md` | `master_plan` | **Decomposição em 4 sub-features atômicas:**<br>• `plans/specs/015-ai-chatbot-mobile/`<br>• `plans/specs/016-voice-dose-registration/`<br>• `plans/specs/017-voice-dose-summary/`<br>• `plans/specs/018-anvisa-interactions-local/` | `migrated` | Decomposição em 4 sub-features atômicas ativadas e criadas em conformidade SQP R-221. |
